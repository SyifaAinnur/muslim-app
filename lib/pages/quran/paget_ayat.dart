...
import 'package:muslims_app/pages/models/quran/response_ayat.dart';
import 'package:progress_indicators/progress_indicators.dart';
import 'package:muslims_app/api/api_service.dart';
import 'package:muslims_app/api/api_url.dart';
import 'package:muslims_app/baseurl/base_app.dart' as BaseApp;
import 'package:muslims_app/helper/remove_glow.dart';

import 'package:muslims_app/baseurl/base_app.dart';
...
class PageAyat extends StatefulWidget {
  ...
  @override
  State<PageAyat> createState() => _PageAyatState();
}
...
class _PageAyatState extends State<PageAyat> {
      ...
      home: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () => Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => PageMain())),
          ),
          actions: [
            IconButton(
              icon: Icon(
  ...
  _getData() async {
    await ApiUrl.ayatRange(_data[BaseApp.Data.suratID], _ayatStart, _ayatEnd).then((value) => _url = value);
    ...
    ApiService().get(
      ...
      callback: (status, message, response) {
        setState(() {
          if (status) {
            ResponseAyat resAyat = ResponseAyat.fromJson(jsonDecode(json.encode(response)));
